...
campaign AS (
    SELECT 
        CONCAT('Кампания № ', campaign_number) AS ads_campaign,
        user_id,
        start_date,
        dt
    FROM campaign_numbers
    WHERE campaign_number IN (1, 2)
)
...
SELECT 
    ...
    ROUND(users::DECIMAL / MAX(users) OVER(PARTITION BY ads_campaign), 2) AS retention
FROM (
    SELECT 
        ads_campaign,
        start_date,
    ...
    FROM campaign
    GROUP BY ads_campaign, start_date, dt
) t
WHERE dt - start_date IN (0, 1, 7)