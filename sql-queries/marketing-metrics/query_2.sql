...
campaign AS (
    SELECT
        ...
        CASE 
            ...
            order_id
    FROM user_actions
    WHERE order_id NOT IN (SELECT order_id FROM user_actions WHERE action='cancel_order') AND DATE_PART('month', time) = 9 
),
prices AS (
    SELECT 
        user_id,
...
SELECT 
    ads_campaign,
    ROUND((SUM(price) - 250000)::DECIMAL / 250000 * 100, 2) AS roi
FROM prices
