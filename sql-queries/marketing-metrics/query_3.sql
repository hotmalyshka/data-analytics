...
prices_table AS (
    SELECT 
        CONCAT('Кампания № ', ads_campaign_number) AS ads_campaign,
        user_id, 
        order_id,
        price, 
        dt
    ...
    WHERE ads_campaign_number IN (1, 2) AND order_id NOT IN (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
)

SELECT 
    ads_campaign,
    CONCAT('Day ', DATE_PART('day', dt) - 1) AS day,
    ROUND(SUM(revenue) OVER(PARTITION BY ads_campaign ORDER BY dt) / users_count, 2) AS cumulative_arppu,
    ROUND(250000::DECIMAL / users_count, 2) AS cac
FROM (
    SELECT ads_campaign, dt, sum(price) AS revenue FROM prices_table GROUP BY ads_campaign, dt
    ) t
        LEFT JOIN (
            SELECT ads_campaign, COUNT(DISTINCT user_id) AS users_count FROM prices_table GROUP BY ads_campaign
        ) t1 USING(ads_campaign) 
